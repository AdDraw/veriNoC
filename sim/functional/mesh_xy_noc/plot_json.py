...
def parse(json_file, nomkdir=False):
    ...
    assert 'pckt_data_w' in dict_json.keys(), f"channel_w not in {json_file}"
  ...
  if loop_json:
    ...
    if change_param == "buff_size":
      for size in buff_size:
        tmp_file_path = pathlib.Path(f"mesh_noc_xy_presynth_3_3_{size}_16.json")
        dict_json.append(parse(tmp_file_path, nomkdir=True))
        dicty, json_no_suffix = parse(tmp_file_path, nomkdir=True)
        df_l.append(pandas.DataFrame.from_dict(dicty["testcases"]))
    elif change_param == "net_size":
      for nxn in nxn_size:
        tmp_file_path = pathlib.Path(f"mesh_noc_xy_presynth_{nxn}_2_16.json")
        dict_json.append(parse(tmp_file_path, nomkdir=True))
        dicty, json_no_suffix = parse(tmp_file_path, nomkdir=True)
        df_l.append(pandas.DataFrame.from_dict(dicty["testcases"]))
    else:
    ...
    fig_size = (6.4, 4.8)
    x = "offered_traffic"
    metric_name = ["accepted_traffic", "avg_packet_latency_ns"]
    ...
    dict_json, json_no_suffix = parse(json_file, nomkdir=False)
    # Gather info
    name = dict_json['noc_type']
    ps = dict_json['ps']
    row_n = dict_json['row_n']
    col_m = dict_json['col_m']
    buffer_w = dict_json['fifo_depth_w']
    channel_w = dict_json['pckt_data_w']
