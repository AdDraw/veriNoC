...
class SWPacketDriver(BusDriver):
    ...
    async def n_rand_data_packets(self, n):
        for i in range(n):
            ...
            self.bus.pckt_sw_i <= getrandbits(self.config["packet_w"] * self.config["neighbours_n"])
    ...
    async def write_to_single_input(self, input_id, x_dest, y_dest):
        ...
        self.bus.pckt_sw_i <= BinaryValue(0, self.config["neighbours_n"]*self.config["packet_w"], bigEndian=False)
        self.bus.wr_en_sw_i <= BinaryValue(0, self.config["neighbours_n"], bigEndian=False)
